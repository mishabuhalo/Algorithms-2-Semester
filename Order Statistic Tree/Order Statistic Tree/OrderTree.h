...
public:
	...
	void repairSizeValue(Node* root) {
		...
		if (root->getLeftChild()) {
			repairSizeValue(root->getLeftChild());
		}
		if (root->getRightChild()) {
			repairSizeValue(root->getRightChild());
		}

		int left = (!root->getLeftChild() ? 0 : root->getLeftChild()->getSize());
		int right = (!root->getRightChild() ? 0 : root->getRightChild()->getSize());
	...
	void print(Node* node, int depth = 0) {
		...
		if (node->getLeftChild()) print(node->getLeftChild(), depth + 1);
